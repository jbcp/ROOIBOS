 ...
 * @author jhlee
 */
public class MyScheduleTableModelHelper_Eng {
    ...
    public String[] title = {"Subject", "Group", "Enable", "1st", "2nd", "3rd", "4th", "5th", "6th",
    ...
    public MyScheduleTableModelHelper_Eng(int rowNum, int colNum) {
...
//      setTitle(setting[Rooibos.COL_DATA_START]);
     ...
     //   periodTiemList.add(0, setting[Rooibos.STANDARD_TODAY_MIN]);
        ...
        for (int j = 0; j < columnNames.length; j++) {
            ...
            if (j == 0) {//대상자번호칼럼 put subjectNumber for colum 0; 
                ...
                int dataCol = j - Rooibos.COL_DATA_START;///대상자, 그룹 칼럼, 활성화칼럼 제와을 제외. j=3 부터 스케쥴 표 시작
   ...
   //System.out.println(setMin + "row " + i + "  col" + j + "   periodTiemList.get(dataCol)" + periodTiemList.get(dataCol) + "  setting[Rooibos.GAP] * i =" + setting[Rooibos.GAP] * i+"==?"+testTimeMilis+setMin*60*1000);
                    ...
                    if (i >= start - 1) {
                        //start check group
                        String groupsOfSample = periodGroupCallList.get(dataCol)[0];//대상자, 그룹 칼럼을 제외
...
//                });
    ...
    public MyScheduleTableModelHelper_Eng(int[] setting, TreeMap<Integer, String[]> periodGroupMap, Date testDate, String[] groupSelectionArr) {
            ...
            for (int i = start - 1; i < numRows; i++) {
              ...
              //
                  //  System.out.println(time.getTime() + "++++++++++++++" + pNum);
                    ...
                    addToHashTable(new Date(date.getTime() - 5000), 55, TYPE_COUNT);//subject Num=i+1 //<--Data to call
                  
                
                    addToHashTable(new Date(date.getTime() - 4000), 56, TYPE_COUNT);//subject Num=i+1 //<--Data to call
                    addToHashTable(new Date(date.getTime() - 3000), 57, TYPE_COUNT);//subject Num=i+1 //<--Data to call
                    addToHashTable(new Date(date.getTime() - 2000), 58, TYPE_COUNT);//subject Num=i+1 //<--Data to call
                    addToHashTable(new Date(date.getTime() - 1000), 59, TYPE_COUNT);//subject Num=i+1 //<--Data to call
                    addToHashTable(date, 00, TYPE_COUNT);//subject Num=i+1 //<--Data to call

                    addToHashTable(new Date(date.getTime() -  prepare*1000), i + 1, TYPE_PREPARE);//subject Num=i+1 //<--Data to call
                    int ts = Integer.parseInt(periodGroupCallList.get(j - Rooibos.COL_DATA_START)[1].trim());
                    if (ts >0) {
                      
                        int call = ts * 60 - callOffset;
                         addToHashTable(new Date(date.getTime() -  call*1000) , i + 1, TYPE_CALL);//subject Num=i+1 //<--Data to call
                    }
                }
...
//    public int timeToMin(String text) {
//        //  textArea.setText( text );
//        String[] tmp = text.split(":");
//        if (tmp[0].charAt(0) == '0') {
//            tmp[0] = "" + tmp[0].charAt(1);
//        }
//        if (tmp[1].charAt(0) == '0') {
//            tmp[1] = "" + tmp[1].charAt(1);
//        }
//
...
//        return time;
//    }
    private void addToHashTable(Date date, int pNum, int type) {
    ...
    //    System.out.println(time.getTime() + "++++++++++++++" + pNum);
        ...
        if (treemap.containsKey(date.getTime())) {
            pList = treemap.get(testTimeSec);

            if (type == 0) {
                return;
            }
            for (int i = 0; i < pList.size(); i++) {
 //System.out.println(i+"\t"+pList.get(i).getType()+"\t "+pList.get(i).getSubject());
                // System.out.println(testTimeSec"++++++++++++++"+pList.get(i).getType()+"type"+ type);
                if (pList.get(i).getType() > type) {

           //         System.out.print(i + " -->  " + pList.get(i).getSubject() + "\t삽입전   " + pList.size());

                    pList.add(i, new ASchedule(type, pNum));

      //              System.out.print(" -->  " + pNum + "\t삽입후   " + pList.size());

                    break;
                } else if (pList.get(i).getType() == type) {
                    //    System.out.println("++++++++++++++"+pList.get(i).getSubject() +"pNum"+ pNum);
       ...
       //                 System.out.print(i + " -->  " + pNum + "\t삽입후   " + pList.size());

                        break;
                    } else if (pList.get(i).getSubject() == pNum) {

                   //     System.out.println("동일존재      " + pNum + "    " + i + " 번째   ");
        ...
        //       System.out.println("------------------");
            ...
            pList.add(aDB);
//            if (type != 0) {
//                System.out.println("new insert      " + aDB.getType() + "\t " + aDB.getSubject());
//            }
        }
...
//        }
        ...
        public String toString(){
            String t_tmp="COUNT";
            if(this.type==1) t_tmp="PREPARE";
            else if(this.type==2) t_tmp="CALL";
            
            return t_tmp+"\tsubject= "+this.subject;
        }
        public boolean equals(int type, int subject){
            if((this.type==type)&&(this.subject==subject))  return true;
            return false;
        }
        public boolean equals(ASchedule aDB2){            
            if((this.type==aDB2.getType())&&(this.subject==aDB2.getSubject()))  return true;
            return false;
        }
